...
export async function generateOTP(username){
  ...
  try {
      ...
      const {data : { code }, status } = await axios.get('http://localhost:8080/api/genrateOTP', { params : { username }});
...
export async function generateOTPemail(email){

  try {

      const {data : { code }, status } = await axios.get('http://localhost:8080/api/generateOTPemail', { params : { email }});
      ...
      if(status === 201){
          ...
          await axios.post('http://localhost:8080/api/registerMail', { userEmail:email, text, subject : "Verify Email OTP"})
...
export async function verifyOTPemail({ code }) {
  try {
    const { data, status } = await axios.get(
      "http://localhost:8080/api/verifyOTPemail",
      { params: {  code } }
    );
